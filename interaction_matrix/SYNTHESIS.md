...
inside the bulk-copper band 0.3–0.5. The relative ordering is physically interpretable (Lomer and Hirth locks strong, coplanar and glissile weaker). The clear anomaly is collinear, which comes out *weakest* though it is canonically strongest: in a probe-through-forest geometry, collinear contact (same Burgers, opposite line) *annihilates* the obstacle, so the local depinning stress is low. This is a genuine definition difference — we measure junction depinning strength, whereas the canonical "collinear strongest" is a mean-free-path/storage effect — and it flattens our matrix relative to the canonical one.
...
1. **Flat matrix / collinear.** Our a_ij are flatter than canonical because the collinear case annihilates in a depinning geometry (depinning- vs storage-strength). The recovered α=0.43 is therefore partly compensation among flattened coefficients; a storage-resolved measurement (evolving forest, mean-free-path) would give the canonical contrasted matrix.
