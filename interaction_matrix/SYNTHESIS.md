...
1. **The per-type matrix is not faithful (most important limitation).** Quantitatively benchmarked against the canonical FCC coefficients (`matrix_vs_canonical.png`), our matrix misses the collinear dominance (canonical 0.62 vs ours 0.13) and over-weights Hirth/self/Lomer (2.3–2.6×); the orderings disagree. The bulk α=0.43 is a coincidental average of inaccurate coefficients, not a faithful reproduction. Causes: collinear annihilation in the depinning geometry, and strain-rate (not quasi-static) configuration-uncontrolled depinning. The prediction-validation loop (§3–4) is still internally valid (the same matrix is used on both sides), but the matrix's fidelity to real FCC physics is the headline open problem; a quasi-static, configuration-controlled protocol (Madec/Devincre style) is required.
