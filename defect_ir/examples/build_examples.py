...
from defect_ir.uncertainty import (
    slip_system_candidates,
    network_assignment_summary,
    assignment_entropy,
)
...
def build_graphene():
    ...
    atoms = [
        {"id": 0, "pos": [48.0, 50.0], "role": "atom", "coordination": 5},
    ...
    for a in atoms:
        doc["geometry"]["vertices"].append(
            {"id": a["id"], "pos": a["pos"], "role": "atom", "constraint": "free"}
        )
    doc["geometry"]["edges"] = [{"id": 0, "v1": 0, "v2": 1, "kind": "core_pair"}]
    doc["topology"]["vertex_labels"] = [
        {
            "vertex_id": a["id"],
            ...
            "defect_type": (
                "5"
                if a["coordination"] == 5
                else "7" if a["coordination"] == 7 else "none"
            ),
        }
        for a in atoms
    ]
    doc["topology"]["edge_labels"] = [
        {
            "edge_id": 0,
            "kind": "5-7_core",
            "slip_system_candidates": [],
    ...
    doc["topology"]["counts"] = {
        "n_vertices": len(atoms),
        "n_edges": 1,
        "n_cores": 1,
        "density_m2": 1.0 / ((L * 2.46e-10) ** 2),
    }
    doc["uncertainty"] = {
        "burgers_assignment": {"method": "topology_5_7_core", "validated_by_gb": False},
        "note": "2D Burgers implied by 5-7 separation; not a 3D vector",
    }
    doc["simulation_targets"] = {
        "engine": "pfc",
        "loading_mode": "affine_tension",
...
def main():
    out = {}
    for name, doc in [
        ("cu_stem_idr", build_cu_stem()),
        ("graphene_defect_idr", build_graphene()),
    ]:
        ok, errs, warns = validate_idr(doc)
        path = os.path.join(HERE, name + ".json")
        json.dump(doc, open(path, "w"), indent=1)
        print(
            f"[{name}] valid={ok}  errors={len(errs)} warnings={len(warns)} -> {os.path.basename(path)}"
        )
        for e in errs:
            print("   ERROR:", e)
        for w in warns:
    ...
    print(
        "assignment uncertainty (Cu):",
        json.dumps(cu["uncertainty"]["assignment_summary"]),
    )
