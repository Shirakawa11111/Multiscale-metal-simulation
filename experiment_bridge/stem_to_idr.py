...
def load_lines():
    ...
    for fp in sorted(glob.glob(os.path.join(RECON, "points_3d*.txt"))):
        pts = [[float(x) for x in ln.split()] for ln in open(fp) if len(ln.split()) == 3]
        if len(pts) >= 2:
...
def build_idr():
    ...
    for ln in lines:
        ...
        for j, p in enumerate(bln):
            ...
            doc["geometry"]["vertices"].append(
                {"id": vid, "pos": [float(p[0]), float(p[1]), float(p[2])],
    ...
    doc["topology"]["counts"] = {"n_vertices": vid, "n_edges": len(edges), "n_lines": len(lines)}
...
def main():
    ...
    print(f"cu_stem_idr.json: valid={ok} errs={len(errs)} warns={len(warns)} "
